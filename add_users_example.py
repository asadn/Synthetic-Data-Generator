...
def get_model_from_json(pattern_file):
    """ Extract Bayesian Tree model from extracted pattern file"""
...
def main():
    pattern_file = "tests/in_data/NF_data_modified.csv_model_old.json"
    header = ['timestamp','in_bytes','out_bytes','l4_dst_port','ipv4_src_addr','ipv4_dst_addr','srcHostName','applicationName','Duration']
    ...
    records = tree_data_proxy.generate_data(_start=datetime.datetime.strptime("2016-04-01 00:00", "%Y-%m-%d %H:%M"),
                            _end=datetime.datetime.strptime("2016-04-10 00:00", "%Y-%m-%d %H:%M"),filename="dummy2.csv")
